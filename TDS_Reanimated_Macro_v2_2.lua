...
--  • Башни во время записи обнаруживаются через ChildAdded
--    → Не зависим от возвращаемого значения RF
--  • Башни во время воспроизведения тоже через ChildAdded
--    → Устранена ошибка "Башня ID:1 не найдена за 45 секунд"
--
--  ПРИНЦИП РАБОТЫ hookmetamethod:
--  ┌──────────────────────────────────────────────────────────┐
--  │  ВСЕ вызовы RF теперь выполняются через OldNamecall()   │
--  │  (без корутин, без изменения timing)                     │
--  │  Запись идёт асинхронно через task.spawn ПОСЛЕ вызова    │
--  └──────────────────────────────────────────────────────────┘
-- ================================================================
...
local Window = Rayfield:CreateWindow({
    ...
    LoadingSubtitle        = "TDS: Reanimated Macro v2.2",
    Theme                  = "Default",
...
local function WatchForNewTower(callback, timeoutSec)
    ...
    conn = TowersFolder.ChildAdded:Connect(function(child)
        ...
        -- Небольшая задержка — Owner мог ещё не реплицироваться
        task.wait(0.15)
...
local function WriteHeader()
    ...
    if #RecordedTroops > 0 then
        table.insert(lines, string.format(
            'TDS:Loadout({"%s"})',
            table.concat(RecordedTroops, '", "')))
    else
        table.insert(lines, '-- TDS:Loadout({}) -- лоадаут не прочитан')
    end
...
OldNamecall = hookmetamethod(game, "__namecall", function(...)
    ...
    if category == "Troops" and action == "Place" then
        ...
        task.spawn(function()
            WatchForNewTower(function(instance)
                TowerCount += 1
                instance.Name = tostring(TowerCount)
    ...
    -- ── "Troops" "Upgrade" ────────────────────────────────────
    --  ID башни берём из Instance.Name (мы назначили его при Place).
    if category == "Troops" and action == "Upgrade" then
        ...
        task.spawn(function()
            ...
            if not id then
                warn("[Recorder] Upgrade: башня не имеет числового ID (Name='"
                    ..tostring(troop.Name).."')")
                return
            end
            local ts = table.concat(timer, ", ")
    ...
    if category == "Troops" and action == "Sell" then
        ...
        task.spawn(function()
            ...
            local id = tonumber(troop.Name)
            if not id then
                warn("[Recorder] Sell: башня не имеет числового ID")
                return
            end
            local ts = table.concat(timer, ", ")
...
Play.Place = function(cmd)
    ...
    WatchForNewTower(function(instance)
        instance.Name = tostring(expectedId)
...
print("══════════════════════════════════════════")
print("  TDS: Reanimated Macro  v2.2.0  загружен")
print("  RF: "..(RemoteFunction and "✔ OK" or "✘ НЕ НАЙДЕНА"))
print("══════════════════════════════════════════")

Notify("TDS: Reanimated Macro v2.2",
