...
local Window = Rayfield:CreateWindow({
    ...
    LoadingSubtitle        = "TDS: Reanimated Macro v2.5",
    Theme                  = "Default",
...
local function ParseFile(filename)
    ...
    for line in content:gmatch("[^\n]+") do
        ...
        -- TDS:Place("Name", x,y,z, wave,min,sec, true/false, rx,ry,rz)
        -- tc может быть "true"/"false" (в кавычках) или true/false (без)
        local nm,x,y,z,wv,mn,sc,rx,ry,rz = line:match(
            'TDS:Place%("([^"]+)",%s*'..
            '([-%.%d]+),%s*([-%.%d]+),%s*([-%.%d]+),%s*'..
            '(%d+),%s*(%d+),%s*([-%.%d]+),%s*'..
            '["\']?[tf][a-le]+["\']?,%s*'..   -- пропускаем tc (true/false)
            '([-%.%d]+),%s*([-%.%d]+),%s*([-%.%d]+)%)')
        if nm then
        ...
        -- TDS:Upgrade(id, wave, min, sec, true/false)
        ...
        -- TDS:Sell(id, wave, min, sec, true/false)
        ...
        -- TDS:Skip(wave, min, sec, true/false)
        ...
        -- TDS:Target(id, "type", wave, min, sec, true/false)
    ...
    -- Лог распарсенных команд
    print(string.format("[Playback] Распарсено: %d команд из '%s'", #commands, filename))
    for i,cmd in ipairs(commands) do
        if cmd.Action == "Place" then
            print(string.format("  [%d] Place %-12s Wave:%s Pos:(%.1f,%.1f,%.1f)",
                i, cmd.TowerName, tostring(cmd.Wave), cmd.X, cmd.Y, cmd.Z))
        else
            print(string.format("  [%d] %s", i, cmd.Action))
        end
    end
    return commands
end
...
-- ════════════════════════════════════════════════════════════════
--  ЛОГИКА ВОСПРОИЗВЕДЕНИЯ — ОЖИДАНИЕ ПО ДЕНЬГАМ
-- ════════════════════════════════════════════════════════════════
--
--  Макрос НЕ ждёт таймер/волну.
--  Каждое действие выполняется как только:
--    1. Хватает денег (Cash >= стоимость)
--    2. Нужная волна наступила (для Skip — чтобы не пропустить раньше)
--
--  Стоимость башен/апгрейдов берётся из игры через RF.
--  Если RF не возвращает стоимость — пробуем поставить каждые 0.5с.

-- Cash объект игрока
local CashValue = LocalPlayer:WaitForChild("Cash", 10)  -- IntValue

local function GetCash()
    return CashValue and CashValue.Value or 0
end

-- Кэш загруженных Stats чтобы не вызывать require каждый раз
local StatsCache = {}

-- Загрузить Stats башни через require
local function GetStats(towerName)
    if StatsCache[towerName] then return StatsCache[towerName] end

    local ok, stats = pcall(function()
        local module = ReplicatedStorage.Assets.Troops[towerName]:FindFirstChild("Stats")
        if module then return require(module) end
    end)

    if ok and stats then
        StatsCache[towerName] = stats
        return stats
    end
    warn("[TDS Macro] Stats не найден для: "..tostring(towerName))
    return nil
end

-- Стоимость постановки башни
local function GetPlaceCost(towerName)
    local stats = GetStats(towerName)
    return stats and stats.Price or nil
end

-- Стоимость следующего апгрейда башни (по текущему уровню)
local function GetUpgradeCost(instance)
    -- Текущий уровень из Upgrade IntValue (0 = не прокачана)
    local upgradeVal = instance:FindFirstChild("Upgrade")
    local currentLevel = upgradeVal and upgradeVal.Value or 0
    local nextLevel = currentLevel + 1

    local typeVal = instance:FindFirstChild("Type")
    if not typeVal then return nil end

    local stats = GetStats(typeVal.Value)
    if not stats or not stats.Upgrades then return nil end

    local upgrades = stats.Upgrades[1]  -- первый (и обычно единственный) путь
    if not upgrades then return nil end

    local upg = upgrades[nextLevel]
    return upg and upg.Cost or nil
end

-- Ждать пока денег >= cost
-- Если cost = nil — пробуем сразу (RF сам проверит деньги)
local function WaitForCash(cost, actionName)
    local startCount = RestartCount

    if not cost then
        print(string.format("[Playback] %s — стоимость неизвестна, пробуем сразу", actionName))
        return true
    end

    if GetCash() >= cost then return true end

    print(string.format("[Playback] Жду деньги для %s: нужно %d, есть %d",
        actionName, cost, GetCash()))

    repeat
        task.wait(0.3)
        if RestartCount ~= startCount then return false end
        if not MacroState.IsPlaying then return false end
    until GetCash() >= cost

    return true
end

-- Ждать нужную волну (только для Skip — чтобы не пропустить раньше времени)
local function WaitForWave(wave)
    local startCount = RestartCount
    repeat
        task.wait(0.5)
        if RestartCount ~= startCount then return false end
    until GetWave() >= wave or not MacroState.IsPlaying
    return MacroState.IsPlaying and RestartCount == startCount
end
...
-- Place: ждём деньги → RF → ChildAdded
Play.Place = function(cmd)
    -- Получаем стоимость
    local cost = GetPlaceCost(cmd.TowerName)

    -- Ждём деньги
    local ok = WaitForCash(cost, string.format('Place "%s"', cmd.TowerName))
    if not ok then return end

    -- Резервируем ID заранее
    TowersContained.Index += 1
    local expectedId = TowersContained.Index
    TowersContained[expectedId] = {
        TowerName = cmd.TowerName,
        Instance  = nil,
        Placed    = false,
    }

    -- Слушаем появление башни
    WatchForNewTower(function(instance)
        ...
        print(string.format("[Playback] ✔ Place  %-12s  ID:%-3d  Cash:%d",
            cmd.TowerName, expectedId, GetCash()))
    end, 15)
    ...
    if not placed then
        warn(string.format("[Playback] Place RF ошибка: %s", tostring(rfErr)))
    end
end

-- Upgrade: ждём башню → ждём деньги → RF
Play.Upgrade = function(cmd)
    ...
    -- Стоимость апгрейда
    local cost = GetUpgradeCost(tower.Instance)
    local ok   = WaitForCash(cost, string.format("Upgrade ID:%d", cmd.Id))
    if not ok then return end
    ...
    print(string.format("[Playback] ✔ Upgrade  ID:%-3d  Cash:%d", cmd.Id, GetCash()))
end

-- Sell: ждём башню → RF (продажа не требует денег)
Play.Sell = function(cmd)
    ...
    print(string.format("[Playback] ✔ Sell   ID:%-3d", cmd.Id))
end

-- Skip: ждём нужную волну → RF
Play.Skip = function(cmd)
    local ok = WaitForWave(cmd.Wave)
    if not ok then return end
    pcall(function()
        RemoteFunction:InvokeServer("Waves", "Skip")
    end)
...
Play.Target = function(cmd)
    ...
    print(string.format("[Playback] ✔ Target  ID:%-3d → %s", cmd.Id, cmd.TargetType))
end

-- Mode: голосуем за сложность (сразу, без ожидания)
...
local function StartPlayback(filename)
    ...
    PlayThread = task.spawn(function()
        ...
        for i, cmd in ipairs(commands) do
            if not MacroState.IsPlaying or RestartCount ~= startCount then
        ...
        -- Ждём пока все параллельные потоки завершатся (каждый ждёт денег)
        repeat
            task.wait(1)
        until activeCount <= 0
          or not MacroState.IsPlaying
          or RestartCount ~= startCount

        if MacroState.IsPlaying and RestartCount == startCount then
...
TabCfg:CreateLabel("TDS: Reanimated Macro  v2.5.0")
...
print("══════════════════════════════════════════")
print("  TDS: Reanimated Macro  v2.5.0  загружен")
print("  RF: "..(RemoteFunction and "✔ OK" or "✘ НЕ НАЙДЕНА"))
print("══════════════════════════════════════════")

Notify("TDS: Reanimated Macro v2.5",
